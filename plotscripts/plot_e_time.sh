...
for cur_pwr in $PWR_LEVELS; do 
    ...
    set grid
    set title '80 MHz 11ac ${cur_pwr} dBm'
    set xlabel 'Time (seconds)' 
    set ylabel 'Throughput (MBits/s)' 
    set xtics 60
    set ytics 10
    set output '${LOGDIR}/${cur_pwr}_ac.eps' 
    set yrange [0:200] 
    plot \\" > tmp.plt
    ...
    for bf in $BFS; do
        for ant in $ANTS; do
            ...
            put_dat $filename
            echo "\"${filename}.dat\" title '${bf} ${ant}' with lines linestyle ${style}, \\" >> tmp.plt
            style=$((style + 1))
        done
    done
    gnuplot tmp.plt
    ps2pdf ${LOGDIR}/${cur_pwr}_ac.eps ${LOGDIR}/${cur_pwr}_ac.pdf
    fllist="$fllist ${LOGDIR}/${cur_pwr}_ac.pdf"
done
pdfunite $fllist ${LOGDIR}/time_ac.pdf
